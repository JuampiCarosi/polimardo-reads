...
import { statusColors, statusLabels } from "../libros/[id]";
...
export default function Challenge() {
    ...
    const getPartialProgress = () => {
        ...
        const readBooks = bookRead?.filter((book) => challenge?.book_ids.includes(book.id) && book.status === 'read').length;
            ...
            <div className="container mx-auto p-6">
                <div className="mx-auto max-w-6xl">
                    <div className="mb-8 space-y-3">
                        <h1 className="text-3xl font-bold text-slate-900">{challenge?.name}</h1>
                        <div className="font-bold">
                            <div className="text-sm">Creador por: {creator?.name}</div>
                        </div>
                        <div className="text-teal-800 text-lg flex gap-2">
                            <Pill
                                color={"blue"}>
                                {challenge?.participants ?? 0} {(challenge?.participants ?? 0) === 1 ? "Participante" : "Participantes"}
                            </Pill>
                            <Pill
                                color={"yellow"}>
                                {challenge?.book_ids?.length ?? 0} {(challenge?.book_ids?.length ?? 0) > 1 ? "Libros" : "Libro"}
                            </Pill>
                        </div>


                        <div className="mt-2 py-5 text-slate-600 grid grid-cols-2">{challenge?.description}</div>
                        ...
                        {typeof id === 'string' && myChallengesIds?.includes(id) && <div className="text-slate-600">Progreso del desafío: {getPartialProgress()}%</div>}
                        <div className="mb-2 flex w-full justify-center">
                            {typeof id === 'string' && myChallengesIds?.includes(id) && (<Progress
                                value={getPartialProgress()}
                            />)}
                    ...
                    <div className="rounded-md border border-slate-200 bg-white px-3 pb-3 pt-1">
                        <Table>
                            ...
                            <TableBody>
                                {books.map((book) => {
                                    const status = bookRead?.find((item) => item.id === book.id)?.status;
                                    return (
                                        <TableRow
