...
export default async function handler(
  ...
  const bookIdsRated = favoriteGenres.map((book) => book.book_id);

    const { data: bookGenres, error: bookGenresError } = await supabase
        .from("book_genres")
        .select("genre_id")
        .in("book_id", bookIdsRated);

    if (bookGenresError || !bookGenres) {
        console.error(bookGenresError);
        return res.status(500).json({ error: bookGenresError.message });
    }

    const genreIds = bookGenres.map((genre) => genre.genre_id);
